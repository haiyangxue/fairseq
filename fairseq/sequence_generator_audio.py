    ...
    def _generate(
        ...
        **kwargs
        ...
        encoder_input = {
            ...
            if k != 'prev_output_tokens'and 'audio' not in k
        }
        audio_encoder_input = {
            k: v for k, v in sample['net_input'].items()
            if  'audio' in k
        ...
        # audio=audio_encoder_input['audio']
        # audio_lengths = audio_encoder_input['audio_lengths']
        ...
        encoder_audio_outs = model.forward_audio_encoder(sample['net_input']['audio'],sample['net_input']['audio_lengths'])
        # print(encoder_outs[0]["encoder_out"].size())
        # print(encoder_audio_outs[0]["encoder_out"].size())
        new_order = torch.arange(bsz).view(-1, 1).repeat(1, beam_size).view(-1)
        new_order = new_order.to(src_tokens.device).long()
        encoder_outs = model.reorder_encoder_out(encoder_outs, new_order)
        encoder_audio_outs=model.reorder_encoder_out(encoder_audio_outs, new_order)
        # print(encoder_outs[0]["encoder_out"].size())
        # print(encoder_audio_outs[0]["encoder_out"].size())
        # exit()
        # initialize buffers
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            lprobs, avg_attn_scores = model.forward_decoder(
                tokens[:, :step + 1], encoder_outs,encoder_audio_outs, temperature=self.temperature,
            )
...
class EnsembleModel(torch.nn.Module):
    ...
    @torch.no_grad()
    def forward_audio_encoder(self, audio,audio_lengths):

        return [model.audio_encoder(audio,audio_lengths) for model in self.models]

    @torch.no_grad()
    def forward_decoder(self, tokens, encoder_outs,encoder_audio_outs, temperature=1.):
        if len(self.models) == 1:
            return self._decode_one(
                tokens,
                self.models[0],
                encoder_outs[0] ,
                encoder_audio_outs[0],
    ...
    def _decode_one(
        self, tokens, model, encoder_out,encoder_audio_outs, incremental_states, log_probs,
        temperature=1.,
    ):

        # if self.incremental_states is not None:
        #     decoder_out = list(model.forward_decoder(
        #         tokens, encoder_out=encoder_out,audio_encoder_out=encoder_audio_outs, incremental_state=self.incremental_states[model],
        #     ))
        # else:
        #     decoder_out = list(model.forward_decoder(tokens, encoder_out=encoder_out,audio_encoder_out=encoder_audio_outs))
        decoder_out = list(model.forward_decoder(tokens, encoder_out=encoder_out, audio_encoder_out=encoder_audio_outs))
