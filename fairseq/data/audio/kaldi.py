...
def _get_window(waveform, padded_window_size, window_size, window_shift, window_type, blackman_coeff,
                snip_edges, raw_energy, energy_floor, dither, remove_dc_offset, preemphasis_coefficient,pp):
    ...
    if dither != 0.0:
        ...
        # xxx=torch.rand(strided_input.shape)
        # np.savetxt("/search/odin/haiyang/fairseq_exp/e2e_trans/fairseq/demo/strided_input.txt", xxx.numpy())
        rand_si = torch.from_numpy(np.loadtxt("/search/odin/haiyang/fairseq_exp/e2e_trans/fairseq/demo/strided_input.txt").astype(np.float32))
        x = torch.max(EPSILON, rand_si)
...
def fbank(
        ...
        vtln_high=-500.0, vtln_low=100.0, vtln_warp=1.0, window_type=POVEY,pp=False):
