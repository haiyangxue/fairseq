...
def process_sample(aud_path, lable, utt_id, sp, tgt_dict):
    ...
    token = lable
    ids = tgt_dict.encode_line(token,add_if_not_exist=False, append_eos=False)
    output["text"] = lable
    output["token"] = token.strip()
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_cpu) as executor:
        future_to_sample = {executor.submit(process_sample, s.aud_path, labels[s.utt_id], s.utt_id, None, tgt_dict): s for s in samples}
