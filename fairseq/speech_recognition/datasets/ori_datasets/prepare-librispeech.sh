...
fairseq_root=/search/odin/haiyang/fairseq_exp/e2e_trans/fairseq
mkdir -p ${out_dir}
cd ${out_dir} || exit
...
path=/search/odin/haiyang/fairseq_exp/e2e_trans/fairseq/examples/speech_recognition/datasets
dict=$path/data/lang_char/${train_dir}_${bpemode}${nbpe}_units.txt
encoded=$path/data/lang_char/${train_dir}_${bpemode}${nbpe}_encoded.txt
fairseq_dict=$path/data/lang_char/${train_dir}_${bpemode}${nbpe}_fairseq_dict.txt
bpemodel=$path/data/lang_char/${train_dir}_${bpemode}${nbpe}
echo "dictionary: ${dict}"
echo "Dictionary preparation"
mkdir -p $path/data/lang_char/
echo "<unk> 3" > ${dict}
echo "</s> 2" >> ${dict}
echo "<pad> 1" >> ${dict}
cut -f 2- -d" " ${download_dir}/LibriSpeech/${train_dir}/text > $path/data/lang_char/input.txt
python $path/sen_piece_train.py --input=$path/data/lang_char/input.txt --vocab_size=${nbpe} --model_type=${bpemode} --model_prefix=${bpemodel} --input_sentence_size=100000000 --unk_id=3 --eos_id=2 --pad_id=1 --bos_id=-1 --character_coverage=1
python $path/sen_piece_encode.py ${bpemodel}.model  $path/data/lang_char/input.txt  ${encoded}
