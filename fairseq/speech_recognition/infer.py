...
def process_predictions(
    args, hypos, tgt_dict, target_tokens, res_files, speaker, id
):
    for hypo in hypos[: min(len(hypos), args.nbest)]:
        hyp_words = tgt_dict.string(hypo["tokens"].int().cpu())
        # hyp_words = sp.DecodePieces(hyp_pieces.split())
        print(
            "{} ({}-{})".format(hyp_words, speaker, id), file=res_files["hypo"]
        )
        # print("{} ({}-{})".format(hyp_words, speaker, id), file=res_files["hypo.words"])

        tgt_words = tgt_dict.string(target_tokens)
        print("{} ({}-{})".format(tgt_words, speaker, id), file=res_files["ref"])
...
def prepare_result_files(args):
    ...
    return {
        "hypo": get_res_file("hypo"),
        # "hypo.units": get_res_file("hypo.units"),
        "ref": get_res_file("ref"),
    ...
    with progress_bar.build_progress_bar(args, itr) as t:
        ...
        for sample in t:
            ...
            for i, sample_id in enumerate(sample["id"].tolist()):
                ...
                process_predictions(
                    args, hypos[i],tgt_dict, target_tokens, res_files, speaker, id
                )
